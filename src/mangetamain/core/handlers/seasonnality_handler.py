...
class SeasonalityHandler(RecipesHandler):
    ...
    def get_periods_all(self, recipe: pd.DataFrame) -> pd.DataFrame:
        ...
        tags = self.get_period_type_column(recipe["tags"], self.ref_names["season"][0])
        name = self.get_period_type_column(recipe["name"], self.ref_names["season"][0])
        ...
        tags = self.get_period_type_column(recipe["tags"], self.ref_names["event"][0])
        name = self.get_period_type_column(recipe["name"], self.ref_names["event"][0])
        description = self.get_period_type_column(
        ...
        final_seasons = seasons.replace("", pd.NA).combine_first(season_from_event)
        final_seasons = final_seasons.fillna("")
