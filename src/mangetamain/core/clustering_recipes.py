...
def vectorize_and_score(
    df: pd.DataFrame, sample_n: int | None = None, random_state: int = 42
) -> pd.DataFrame:
    ...
    src["_proc_ings"] = src.get("ingredients", pd.Series([[]] * len(src))).apply(
        ingred_tokenizer
    )
...
def _string_to_list(tags_string: object) -> list:
    ...
    try:
        ...
        try:
            return [
                t.strip()
                for t in re.split(r",\s*", tags_string.strip("[]"))
                if t.strip()
            ]
        except Exception:
            return []
...
def extract_time_tags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = pd.DataFrame(
        {
            "id": df["id"].values,
            "time_tags": time_tags.values,
            "time_tags_str": time_tags_str.values,
        }
    )
    return out
...
@st.cache_data(show_spinner=False)
def compute_time_tag_similarity(
    df: pd.DataFrame, sample_n: int | None = None, random_state: int = 1
) -> pd.DataFrame:
    ...
    if sample_n is not None and sample_n > 0 and len(tags_df) > sample_n:
        tags_df = tags_df.sample(n=sample_n, random_state=random_state).reset_index(
            drop=True
        )
...
def compute_similarity_from_dataset(
    anchor: Path | str, sample_n: int | None = None, random_state: int = 1
) -> pd.DataFrame:
...
def save_time_similarity_matrix(
    ...
    """
    # Compute similarity
    sim_df = compute_similarity_from_dataset(
        anchor, sample_n=sample_n, random_state=random_state
    )
    ...
    print(
        f"Saved time-tag similarity matrix to: {out_path} ({sim_df.shape[0]}×{sim_df.shape[1]})"
    )
    return out_path
