...
class BubbleMapFolium:
    ...
    def __init__(
        ...
        if country_centroids is not None:
            self._country_centroids = country_centroids.rename(columns=str.lower)[
                ["country", "lat", "lon"]
            ].copy()
    ...
    def counts_by_country(
        df_raw: pd.DataFrame, country_col: str = "country"
    ) -> pd.DataFrame:
        return (
    ...
    @staticmethod
    def counts_by_continent(
        df_raw: pd.DataFrame, continent_col: str = "continent"
    ) -> pd.DataFrame:
        return (
    ...
    @staticmethod
    def _scale_sizes(
        counts: np.ndarray, min_px: float, max_px: float, use_sqrt: bool = True
    ) -> np.ndarray:
    ...
    def _load_world_gdf(self):
        ...
        try:
            ...
            keep = [
                c
                for c in ["name", "name_long", "name_en", "continent", "geometry"]
                if c in gdf.columns
            ]
            gdf = gdf[keep].copy()
            ...
            try:
                ...
                gdf = gpd.GeoDataFrame(
                    columns=["name", "name_long", "name_en", "continent", "geometry"],
                    geometry="geometry",
                    crs="EPSG:4326",
                )
        ...
        if "name_en" in gdf.columns and gdf["name_en"].notna().any():
            ...
            base = (
                ...
                if "name" in gdf.columns
                    ...
                    gdf.columns.intersection(["admin", "NAME", "NAME_LONG"]).tolist()
                    or ["name"]
                )[0]
            )
    ...
    def build_map(
        ...
        # validate input
        req = (
            ["country", "count_recipes"]
            if level == "country"
            else ["continent", "count_recipes"]
        )
        ...
        if level == "country":
            left = work.copy()
            left["country_norm"] = (
                left["country"].astype(str).str.strip().str.casefold()
            )
            right = self._country_centroids.copy()
            right["country_norm"] = (
                right["country"].astype(str).str.strip().str.casefold()
            )
            work = left.merge(
                right[["country_norm", "lat", "lon"]], on="country_norm", how="left"
            )
                ...
                if gdf is not None:
                    folium.GeoJson(
                        ...
                        tooltip=folium.GeoJsonTooltip(
                            fields=["name"], aliases=["Country"], sticky=False
                        ),
                        popup=folium.GeoJsonPopup(
                            fields=["name"], labels=False, sticky=False
                        ),
                        control=False,
                    ).add_to(m)
                    ...
                    if selected_country and zoom_on_selected:
                        ...
                        if not match.empty:
                            if (
                                zoom_mode.lower() == "continent"
                                and "continent" in gdf.columns
                            ):
        ...
        for (_, row), r_px in zip(work.iterrows(), radii_px):
            ...
            popup = folium.Popup(
                f"<b>{row[label_col]}</b><br/>Total : {value}", max_width=250
            )
