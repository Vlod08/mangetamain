...
if tags_count is None:
    ...
    if "tags" in df.columns:
        ...
        tags_count = pd.Series(
            [t for row in df["tags"].dropna().map(_split_tags) for t in row]
        ).nunique()
    else:
        tags_count = 0
...
st.info(
    "Naviguez via le menu **Pages** (à gauche) : qualité, exploration, pays, saisons, clustering, etc."
)
