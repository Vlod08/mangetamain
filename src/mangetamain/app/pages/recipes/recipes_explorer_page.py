...
from mangetamain.app.app_utils.viz import kde_plot
...
def add_filters(recipes_eda_svc: RecipesEDAService):
    ...
    minutes_ranges = {
        f"All        ({minutes_to_period(m_lo)} - {minutes_to_period(m_hi)})": (
            m_lo,
            m_hi,
        ),
        f"Very short ({minutes_to_period(m_lo)} - {minutes_to_period(minutes_percentiles_25)})": (
            m_lo,
            minutes_percentiles_25,
        ),
        f"Short      ({minutes_to_period(minutes_percentiles_25)} - {minutes_to_period(minutes_percentiles_75)})": (
            minutes_percentiles_25,
            minutes_percentiles_75,
        ),
        f"Medium     ({minutes_to_period(minutes_percentiles_75)} - {minutes_to_period(minutes_percentiles_80)})": (
            minutes_percentiles_75,
            minutes_percentiles_80,
        ),
        f"Long       ({minutes_to_period(minutes_percentiles_80)} - {minutes_to_period(minutes_percentiles_90)})": (
            minutes_percentiles_80,
            minutes_percentiles_90,
        ),
        f"Very long  ({minutes_to_period(minutes_percentiles_90)} - {minutes_to_period(minutes_percentiles_95)})": (
            minutes_percentiles_90,
            minutes_percentiles_95,
        ),
        f"Extra long (> {minutes_to_period(minutes_percentiles_95)})": (
            minutes_percentiles_95,
            m_hi,
        ),
        "Custom range": None,
    ...
    if selected_label == "Custom range":
        ...
        min_minutes = st.number_input(
            "Min minutes", min_value=m_lo, max_value=m_hi, value=m_lo, step=1
        )
        max_minutes = st.number_input(
            "Max minutes",
            min_value=min_minutes,
            max_value=m_hi,
            value=minutes_percentiles_90,
            step=1,
        )
    ...
    df_filtered = recipes_eda_svc.ds.df[
        recipes_eda_svc.ds.df["minutes"].between(*selected_range)
    ]
    ...
    st.plotly_chart(kde_plot(df_filtered, "minutes"), config={"width": "stretch"})
    st.write(
        f"Selected range: {minutes_to_period(minutes_range[0])} → {minutes_to_period(minutes_range[1])}"
    )

    # --- N_STEPS FILTER ---
    steps_range = st.slider(
        "Select n_steps Range", min_value=s_lo, max_value=s_hi, value=(s_lo, s_hi)
    )
    df_filtered = df_filtered[df_filtered["n_steps"].between(*steps_range)]
    st.plotly_chart(kde_plot(df_filtered, "n_steps"), config={"width": "stretch"})
