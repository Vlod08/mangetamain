...
def app():
    ...
    df_clust["text"] = (
        df_clust.get("name", pd.Series([""] * len(df_clust))).fillna("")
        + " "
        + df_clust.get("ingredients", pd.Series([""] * len(df_clust))).astype(str)
        + " "
        + df_clust.get("description", pd.Series([""] * len(df_clust))).fillna("")
    ).str.lower()
    ...
    st.dataframe(
        cr.top_terms_per_cluster(km, tfidf, topn=8), width="stretch", hide_index=True
    )
    ...
    st.markdown(
        "Calculate recipe similarity using only ingredients (Count vectorization + cosine similarity). Choose a random sample, select a recipe, and display the most similar recipes based on ingredients."
    )
    ...
    with c2:
        seed = st.number_input(
            "Random seed", min_value=0, max_value=9999, value=5, step=1, key="ing_seed"
        )
    with c3:
    ...
    if sim.empty:
        st.warning(
            "No ingredient tokens found in the sample. Try increasing the sample size or check the 'ingredients' column."
        )
    else:
        # Selection UI
        options = [
            f"{rid} — {str(name)[:80]}"
            for rid, name in zip(
                df_sim["id"], df_sim.get("name", pd.Series([None] * len(df_sim)))
            )
        ]
        sel = st.selectbox(
            "Choose a recipe (from the sample)", options, key="ing_select"
        )
        ...
        with st.spinner("Calculating nearest neighbours (ingredients)..."):
            try:
                neighbours = ci.find_similar_by_ingredients(
                    sim, selected_id, top_n=top_n
                )
            except KeyError:
                st.error(
        ...
        if neighbours.empty:
            ...
            neigh_df = neigh_df[
                ["id", "score"] + [c for c in display_cols if c not in ("id")]
            ]
            ...
            if not neigh_df.empty:
                inspect_id = st.selectbox(
                    "Inspect neighbour", neigh_df["id"].astype(str), key="ing_inspect"
                )
                insp_row = neigh_df[neigh_df["id"].astype(str) == str(inspect_id)].iloc[
                    0
                ]
    ...
    st.markdown(
        "This section calculates the similarity of recipes using time-related tags (e.g. '15-minutes'). Select a random sample, choose a recipe, and display its nearest neighbours based on the cosine similarity of the time tags."
    )
    ...
    with tc1:
        sample_n_tags = st.number_input(
            "Sample size (random subset)",
            min_value=10,
            max_value=50000,
            value=15000,
            step=10,
    ...
    with tc2:
        seed_tags = st.number_input(
            "Random seed", min_value=0, max_value=9999, value=5, step=1, key="tags_seed"
        )
    with tc3:
    ...
    if sim_time.empty:
        st.warning(
            "No time tag found in the sampled recipes. Try a larger sample or check that the 'tags' column contains time tokens."
        )
    else:
        ...
        options_t = [
            ...
            for rid, name in zip(
                sampled_tags["id"],
                sampled_tags.get("name", pd.Series([None] * len(sampled_tags))),
            )
        ]
        sel_t = st.selectbox(
            "Choose a recipe (from the sample)", options_t, key="tags_select"
        )
        ...
        with st.spinner("Loading similar recipes (time tags)..."):
            try:
                neigh_t = ctt.get_similar_recipes(
                    sim_time, selected_id_t, top_n=top_n_tags
                )
            except KeyError:
                st.error(
                    "The selected recipe is not present in the similarity matrix (it may not have time tags)."
                )
                st.stop()
        ...
        if neigh_t.empty:
            ...
            neigh_df_t = neigh_df_t[
                ["id", "score"] + [c for c in display_cols_t if c not in ("id")]
            ]
            ...
            if not neigh_df_t.empty:
                inspect_id_t = st.selectbox(
                    "Inspect neighbour (time tags)",
                    neigh_df_t["id"].astype(str),
                    key="tags_inspect",
                )
                insp_row_t = neigh_df_t[
                    neigh_df_t["id"].astype(str) == str(inspect_id_t)
                ].iloc[0]
    ...
    st.markdown(
        "Calculate the similarity of recipes using nutritional values. The module attempts to normalize nutritional columns (mapping alternative names or parsing the 'nutrition' column) before computing similarity."
    )
    ...
    with nc1:
        nutri_sample_n = st.number_input(
            "Sample Size",
            min_value=10,
            max_value=50000,
            value=10000,
    ...
    with nc3:
        nutri_top_n = st.slider(
            "Show Top N Similar Recipes", 1, 50, 10, key="nutri_top_n"
        )
    ...
    with st.spinner("Loading and sampling recipes (nutrients)..."):
        ...
        df_nut_norm, nut_info = cn.normalize_nutrition_columns(
            df_nut, try_parse_nut_column=True
        )

        # build similarity on the normalized dataframe
        sim_nut = cn.build_nutri_similarity(
            df_nut_norm, sample_n=None, random_state=int(nutri_seed)
        )
    ...
    if sim_nut.empty:
        ...
        present = (
            nut_info.get("present", [])
            if isinstance(nut_info, dict)
            else [c for c in cn.NUTRI_COLS if c in df_nut_norm.columns]
        )
        alt_present = (
            nut_info.get("alt_present", []) if isinstance(nut_info, dict) else []
        )
        ...
        sel_n = st.selectbox(
            "Choose a recipe (in the sample)", options_n, key="nutri_select"
        )
        ...
        with st.spinner("Calculating nutritional similarity..."):
            try:
                neighbours_n = cn.find_similar_by_nutri(
                    sim_nut, selected_id_n, top_n=nutri_top_n
                )
            except KeyError:
                st.error(
                    "Selected recipe not found in the nutritional similarity matrix."
                )
        ...
        if neighbours_n.empty:
            ...
            if not neigh_df_n.empty:
                inspect_id_n = st.selectbox(
                    "Inspect similar recipe (nutrients)",
                    neigh_df_n["id"].astype(str),
                    key="nutri_inspect",
                )
                insp_row_n = neigh_df_n[
                    neigh_df_n["id"].astype(str) == str(inspect_id_n)
                ].iloc[0]
                st.markdown(f"### {insp_row_n.get('name','—')}")
                st.write(
                    {
                        k: insp_row_n.get(k)
                        for k in ["minutes", "n_steps"] + nutri_cols
                        if k in insp_row_n.index
                    }
                )
