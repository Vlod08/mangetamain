...
def app():
    ...
    if not logs_dir.exists():
        st.info("No directory **logs/** found. Logs will be created automatically as soon as an event is logged.")
        st.stop()
    ...
    with st.sidebar:
        ...
        level_filter = st.multiselect("Levels", ["INFO", "WARNING", "ERROR", "CRITICAL", "DEBUG"], default=[])
    ...
    colB.metric("Size", f"{cur_log.stat().st_size/1024:.1f} Ko" if cur_log.exists() else "—")
    colC.metric("Displayed lines", len(df))
    colD.metric("Modified", time.strftime("%H:%M:%S", time.localtime(cur_log.stat().st_mtime)) if cur_log.exists() else "—")
    ...
    with btn_cols[1]:
        if st.button("🧹 Purge file", type="secondary"):
            ...
            st.success(f"{file_choice} emptied."); st.experimental_rerun()
    with btn_cols[2]:
