...
def app():
    ...
    with st.sidebar:
        ...
        if uploaded_file is None:
            st.info(
                "Using default dataset (Food.com reviews). You can upload your own CSV/Parquet file."
            )
        else:
        ...
        if st.button("🧹 Regenerate Clean Artefact"):
            ...
            st.success("Artefact regenerated successfully.")
    ...
    dates_ok = ("date" in interactions_eda_svc.ds.df.columns) and pd.to_datetime(
        interactions_eda_svc.ds.df["date"], errors="coerce"
    ).notna().any()
    ...
    with tabs[2]:
        ...
        fdf = interactions_eda_svc.apply_filters(
            rating_range=rating_range, min_len=min_len, year=year_opt
        )
