...
st.markdown("Compute recipe similarity using ingredients only (Count vectorization + cosine similarity). Pick a random sample, select a recipe and see the most similar recipes by ingredients.")
...
sample_n = st.sidebar.number_input("Sample size", min_value=50, max_value=50000, value=5000, step=50)
...
if sim.empty:
    st.warning("No ingredient tokens found in the sampled data. Try increasing the sample size or check the 'ingredients' column.")
    st.stop()

# Selection UI
options = [f"{rid} — {str(name)[:80]}" for rid, name in zip(df["id"], df.get("name", pd.Series([None]*len(df))))]
