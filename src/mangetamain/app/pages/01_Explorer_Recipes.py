...
use_global_ui(
    ...
    logo="image/image.jpg", logo_size_px=90, round_logo=True, wide=True,
)
...
with st.sidebar:
    ...
    inc_tags = [t.strip() for t in st.text_input("Inclure tags ( , )", "").split(",") if t.strip()]
    exc_tags = [t.strip() for t in st.text_input("Exclure tags ( , )", "").split(",") if t.strip()]
    inc_ings = [t.strip() for t in st.text_input("Contient ingrédients ( , )", "").split(",") if t.strip()]

# --------- KPIs ----------
c1,c2,c3,c4 = st.columns(4)
c1.metric("Lignes", f"{len(df):,}")
c2.metric("Colonnes", df.shape[1])
c3.metric("% minutes NA", f"{(df['minutes'].isna().mean()*100):.1f}%" if "minutes" in df else "—")
c4.metric("Dates parsées", "✅" if "submitted" in df and df["submitted"].notna().any() else "—")
...
# ---- Exploration ----
with tab2:
    fdf = svc.apply_filters(minutes=minutes, steps=steps,
                            include_tags=inc_tags, exclude_tags=exc_tags, include_ings=inc_ings)
    st.caption(f"{len(fdf):,} recettes après filtres")

    c1, c2 = st.columns(2)
    hmin = svc.minutes_hist()
    if not hmin.empty:
        c1.plotly_chart(px.bar(hmin, x="left", y="count", title="Distribution des minutes"),
                        use_container_width=True)
    hstp = svc.steps_hist()
    if not hstp.empty:
        c2.plotly_chart(px.bar(hstp, x="left", y="count", title="Distribution des étapes"),
                        use_container_width=True)

    byy = svc.by_year()
    if not byy.empty:
        st.plotly_chart(px.line(byy, x="year", y="n", title="Recettes par année"), use_container_width=True)

    top_ing = svc.top_ingredients(30)
    if not top_ing.empty:
        st.subheader("Top ingrédients")
        st.plotly_chart(px.bar(top_ing.head(20), x="ingredient", y="count"), use_container_width=True)

# ---- Table ----
with tab3:
    cols = [c for c in ["name","minutes","n_steps","n_ingredients","tags"] if c in df.columns]
    view = svc.apply_filters(minutes=minutes, steps=steps,
                             include_tags=inc_tags, exclude_tags=exc_tags, include_ings=inc_ings)
    st.dataframe(view[cols].head(1000), use_container_width=True, hide_index=True)
    st.download_button("⬇️ Export CSV (filtres)", view.to_csv(index=False).encode("utf-8"),
