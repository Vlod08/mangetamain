...
sample_n = st.sidebar.number_input("Sample size (random subset)", min_value=10, max_value=50000, value=15000, step=10)
...
if sim_df.empty:
    st.warning("No time-tags found in the sampled recipes. Try a larger sample or check that tags exist and contain time-related tokens.")
    st.stop()


# Selection UI: show friendly labels
options = [f"{rid} — {str(name)[:80]}" for rid, name in zip(sampled["id"], sampled.get("name", pd.Series([None]*len(sampled))))]
...
with st.spinner("Computing similar recipes..."):
    try:
        ...
        st.error("Selected recipe is not present in the similarity matrix (maybe it has no time-tags).")
        st.stop()
...
if neigh.empty:
    ...
    display_cols = [c for c in ["id", "name", "minutes", "n_steps", "tags"] if c in neigh_df.columns]
    ...
    if not neigh_df.empty:
        ...
        st.write({k: insp_row.get(k) for k in ["minutes", "n_steps", "tags"] if k in insp_row.index})
