...
if not LOG_DIR.exists():
    st.info(
        "Aucun dossier **logs/** trouvé. Les logs seront créés automatiquement dès qu'un événement est journalisé."
    )
    st.stop()
...
with st.sidebar:
    ...
    autorefresh = st.checkbox(
        "Auto-refresh", value=True, help="Rafraîchit toutes les 5s"
    )
    if autorefresh:
    ...
    level_filter = st.multiselect(
        "Niveaux", ["INFO", "WARNING", "ERROR", "CRITICAL", "DEBUG"], default=[]
    )
    query = st.text_input("Recherche (contient) :", placeholder="mot-clé, module, etc.")
    max_lines = st.slider(
        "Dernières lignes", min_value=200, max_value=10000, value=2000, step=200
    )
...
colB.metric(
    "Taille", f"{cur_log.stat().st_size/1024:.1f} Ko" if cur_log.exists() else "—"
)
